...
def main():
    ...
    # Loading screen
    screen.fill((10, 8, 5))
    loading = fonts['big'].render("Génération du monde…", True, (200, 180, 120))
    screen.blit(loading, (SCREEN_W // 2 - loading.get_width() // 2,
                           SCREEN_H // 2 - loading.get_height() // 2))
    pygame.display.flip()

    tiles, _, _ = generate_terrain(seed=42)
    sim    = Simulation(tiles)
    camera = Camera()
    tick   = 0.0
